import Daoforge from "../contracts/soroban_daoforge";
import { deriveProjectKey } from "../utils/projectKey";
import { Buffer } from "buffer";
import { loadedProjectId } from "./StateService";
import { modifyProposalFromContract } from "utils/utils";
import type { Project, Proposal, Member, Badges } from "../../packages/daoforge";
...
async function getProjectHash(): Promise<string | null> {
  ...
  try {
    const res = await Daoforge.get_commit({
...
async function getProject(): Promise<Project | null> {
  ...
  try {
    const res = await Daoforge.get_project({
      project_key: projectKey,
    });
...
async function getProposalPages(project_name: string): Promise<number | null> {
  ...
  try {
    const checkPageExist = async (page: number) => {
      try {
        const res = await Daoforge.get_dao({
          project_key,
          page,
        });
...
async function getProposal(
  ...
  try {
    const res = await Daoforge.get_proposal({
      project_key: project_key,
...
async function getMember(memberAddress: string): Promise<Member | null> {
  ...
  try {
    const res = await Daoforge.get_member({
...
async function getBadges(): Promise<Badges | null> {
  ...
  try {
    ...
    const res: any = await (Daoforge as any).get_badges({ key: projectKey });
...
async function getProjectsPage(page: number): Promise<Project[]> {
  try {
    const res = await Daoforge.get_projects({ page });
    checkSimulationError(res);
...
export async function hasAnonymousVotingConfig(
  ...
  try {
    const project_key = deriveProjectKey(projectName);
    const tx = await (Daoforge as any).get_anonymous_voting_config({
      project_key,
    });
    try {
