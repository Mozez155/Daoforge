...
async function createSignedProposalTransaction(
  ...
  Daoforge.options.publicKey = publicKey;
  const project_key = deriveProjectKey(projectName);

  const tx = await Daoforge.create_proposal({
    proposer: publicKey,
...
async function createSignedAddMemberTransaction(
  ...
  Daoforge.options.publicKey = address;

  const tx = await Daoforge.add_member({
...
async function createSignedUpdateMemberTransaction(
  ...
  const tx = await Daoforge.update_member({
    member_address: address,
    meta: meta,
  });
...
export async function createProjectFlow({
  ...
  const tx = await Daoforge.register({
...
async function createSignedUpdateConfigTransaction(
  ...
  const tx = await Daoforge.update_config({
    maintainer: publicKey,
    key: projectKey,
