...
use crate::{
    DaoTrait, MembershipTrait, Daoforge, DaoforgeArgs, DaoforgeClient, DaoforgeTrait, errors, events, types,
};
...
impl DaoTrait for Daoforge {
    ...
    fn revoke_proposal(env: Env, maintainer: Address, project_key: Bytes, proposal_id: u32) {
        ...
        let admins_config = Daoforge::get_admins_config(env.clone());
    ...
    fn vote(env: Env, voter: Address, project_key: Bytes, proposal_id: u32, vote: types::Vote) {
        ...
        if proposal.vote_data.token_contract.is_none() {
            let voter_max_weight = <Daoforge as MembershipTrait>::get_max_weight(
                env.clone(),
                project_key.clone(),
                vote_address.clone(),
            );
