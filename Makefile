...
ifndef wasm
	override wasm = target/wasm32v1-none/release/daoforge.wasm
endif
...
override daoforge_id = $(shell cat .stellar/daoforge_id-$(network))
...
# --contract-id $(daoforge_id-$(network))
contract_bindings: contract_build  ## Create bindings
	stellar contract bindings typescript \
		--network $(network) \
		--wasm $(wasm) \
		--output-dir dapp/packages/daoforge \
		--overwrite && \
	cd dapp/packages/daoforge && \
	bun install --latest && \
	bun run build && \
	cd ../../.. && \
	stellar contract bindings typescript \
...
contract_deploy:  ## Deploy Soroban contract
	stellar contract deploy \
  		...
  		--network $(network) \
  		--salt $(shell printf daoforge | openssl sha256 | cut -d " " -f2) \
  		--inclusion-fee 200000000 \
  		--cost \
  		-- \
  		--admin $(shell stellar keys address $(admin)) \
  		> .stellar/daoforge_id-$(network) && \
  	cat .stellar/daoforge_id-$(network)
...
contract_register:
	stellar contract invoke \
    	...
    	--url https://github.com/Consulting-Manao/daoforge \
...
pre_push_hook:
	DAOFORGE_CONTRACT_ID=$(daoforge_id) \
	DAOFORGE_PROJECT_KEY=37ae83c06fde1043724743335ac2f3919307892ee6307cce8c0c63eaa549e156 \
	uv run --with soroban pre-commit/daoforge_pre_push.py
