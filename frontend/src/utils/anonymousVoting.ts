// Utility helpers to handle Soroban Daoforge anonymous voting flows
...
import { Badge } from "../../packages/daoforge/dist";
// Lazy-loaded imports to avoid circular dependency issues in Astro/SSR
async function getDaoforge() {
  const mod = await import("../contracts/soroban_daoforge");
  return mod.default;
}
...
export async function validateAnonymousKeyForProject(
  ...
  const Daoforge = await getDaoforge();
  const project_key = deriveProjectKey(projectName);
  try {
    const { result: cfg } = await Daoforge.get_anonymous_voting_config({
      project_key,
    });
...
export async function computeAnonymousVotingData(
  ...
  const { result: rawProposal } = await Daoforge.get_proposal({
    project_key,
  ...
  for (const vote of votes) {
    ...
    if (memberAddr === proposerAddr) {
      ...
      try {
        const maxRes = await Daoforge.get_max_weight({
          project_key,
  ...
  if (verifyProof) {
    try {
      const proofRes = await Daoforge.proof({
        project_key,
