...
function getGlobalIpfsCache(): IpfsCache {
  ...
  const w = window as Window & { __DAOFORGE_IPFS_CACHE__?: IpfsCache };
  if (!w.__DAOFORGE_IPFS_CACHE__) {
    w.__DAOFORGE_IPFS_CACHE__ = { responses: {}, toml: {}, json: {} };
  }
  return w.__DAOFORGE_IPFS_CACHE__;
}
...
const DAOFORGE_TOML_PATH = "/daoforge.toml";

/**
 * Fetch and parse project daoforge.toml from IPFS.
 */
export async function fetchTomlFromIpfs(
  ...
  try {
    const text = await fetchTextFromIpfs(cid, DAOFORGE_TOML_PATH, options);
